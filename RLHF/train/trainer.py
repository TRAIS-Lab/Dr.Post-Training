...
class LayerWiseSubsetPPOTrainer:
    ...
    def __init__(
        ...
        self.selection_strategy = create_separate_batch_strategy(
            ...
            scoring_method=getattr(self.args, 'scoring_method', 'pip'),
    ...
    def _reduce_train_loss(self, per_token_loss: Tensor, mask: Tensor) -> Tensor:
        ...
        "token_mean": token mean over the micro-batch.
        """
    ...
    def _val_loss_contribution(self, per_token_loss: Tensor, mask: Tensor, full_batch_size: int) -> Tensor:
        ...
        responses. "sample_mean": per-response token mean; "token_mean":
    ...
    def _capture_validation_gradients_core(
        ...
        for i in range(0, full_batch_size, batch_size):
            ...
            # token sum under "token_mean"), averaged over the full val batch.
    ...
    def _capture_tokenpg_validation_gradients(
        ...
        token mean under "sample_mean", token sum under "token_mean") and
    ...
    def _capture_ppo_validation_gradients(
        ...
        for i in range(0, full_batch_size, batch_size):
            ...
            if self.loss_reduction == "sample_mean":
                ...
                # "token_mean": token means over the mini-batch, averaged over mini-batches
    ...
    def _compute_initial_stats(
        ...
        stats = {
            ...
            # Reward stats under the reward/ prefix
    ...
    def iif_pre_select(
        ...
        """
        Pre-select samples using IIF (Iterative Influence-based Filtering).
        ...
        for mb_start in range(0, batch_size, mini_batch_size):
            ...
            mb_state = GlobalSubsetState(
                ...
                selection_mode="filtering",
                # Must match the backend the target gradients were captured with: with
                # compressed scoring the cache holds compressed target gradients only, and the
                # default exact path would find no full gradient, skip every layer, and leave
                # all scores at zero (=> "all positive" => nothing filtered).
                scoring_method=getattr(self.args, 'scoring_method', 'pip'),
            )

            # Set item counts for this mini-batch (ones under "sample_mean": one item per
            # response; response token counts under "token_mean"). Only the
    ...
    def compute_ppo_loss(
        ...
        # token mean over the micro-batch.
    ...
    def training_step(
        ...
        # a valid position), so the mask choice below only matters for "token_mean":
