...
class _ValCacheWrapper:
    """Wrapper for API compatibility with base gradstream's _val_cache."""
...
class GradientHookVerl:
    ...
    def _custom_linear_forward(self, module: nn.Linear, idx: int, input: Tensor) -> Tensor:
        ...
        if isinstance(state, GREATSStateVerl):
            return GREATSLinearBackwardVerl.apply(
                input, module.weight, module.bias, self, idx
            )
        elif isinstance(state, StreamingStateVerl):
            return StreamingLinearBackwardVerl.apply(
                input, module.weight, module.bias, self, idx
            )
    ...
    def setup_selection(
        ...
        if selection_method == "GREATS":
            self.selection_state = GREATSStateVerl(
            ...
            self.selection_state = StreamingStateVerl(
                train_batch_size=train_batch_size,
                num_layers=num_layers,
                frac=frac,
                lr=lr,
    ...
    # Property for API compatibility with base gradstream
    @property
...
class StreamingLinearBackwardVerl(Function):
    """Streaming backward with packed sequence support."""
    ...
    def backward(ctx, grad_output):
        ...
        state: StreamingStateVerl = hook_manager.selection_state
        if state is None:
            return grad_input, None, None, None, None
...
class GREATSLinearBackwardVerl(Function):
    """GREATS backward with packed sequence support."""
    ...
    def backward(ctx, grad_output):
        ...
        state: GREATSStateVerl = hook_manager.selection_state
        if state is None:
            return grad_input, None, None, None, None
