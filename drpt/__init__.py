...
Architecture overview:
  ...
  Curation (selection/)
    ...
    Scoring methods: pip (per-token inner product, default), gip (ghost inner product),
    direct, compress.
    Both use score_compressors for influence scoring when configured.

  Fused kernels (kernels/)
    CuTe DSL kernels (Triton port as fallback) behind the scoring and
    curated-w.grad steps of the custom Linear backward (ghost / per-token inner
    product, index-mapped selected w.grad).  Used automatically for 3-D CUDA
    bf16/fp16 tensors; DRPT_KERNEL_BACKEND=cute|triton|off selects the backend.
  ...
  Loss convention (losses.py)
    ...
    selected samples, as in the paper. "token_mean" is the Hugging Face
