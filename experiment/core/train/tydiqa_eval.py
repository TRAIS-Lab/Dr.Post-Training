...
from core.data_selection.get_validation_dataset import get_tydiqa_dataset_df
...
from core.train.eval_utils import generate_completions
...
def compute_accuracy(args, model, tokenizer):
    ...
    if args.n_shot > 0:
        # experiment/core/data/eval/tydiqa/dev/tydiqa-one-shot.json
...
def main(args):
    ...
    eval_scores["average"] = {metric: torch.tensor(
        [scores[metric] for scores in eval_scores.values()], dtype=torch.float32).mean().item() for metric in ["f1", "exact_match"]}
