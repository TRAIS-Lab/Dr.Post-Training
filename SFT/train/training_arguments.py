...
class TrainingArguments(TA):
    ...
    score_normalization: str = field(
        ...
        metadata={
            "help": (
                "Layer-normalized global scoring control: 'none' (default), 'layer_meanabs' or 'layer_std' "
                "rescale each layer's score vector (by its mean absolute value or its std) before the "
                "layers are summed into the shared Global/Group-Wise score. No effect on LayerWiseSubset."
            )
        },
    )
    selection_mode: str = field(
        ...
        metadata={
            "help": (
                "Selection mode: 'topk' (select top frac samples by score), "
                "'filtering' (drop bottom frac of negative-score samples), or 'random' "
                "(Random Subset control: a uniformly random frac of the batch per selection, "
                "shared by all layers under GlobalSubset and drawn independently per layer under "
                "LayerWiseSubset, through the same gradient path as the curated arms)."
            )
        },
    )
    n_val: int = field(
    ...
    scoring_method: str = field(
        ...
        metadata={
            "help": (
                ...
                "'pip' (default; per-token inner product against the aggregated target gradient, "
                "never materializes per-sample grads), "
                "'gip' (GREATS-style ghost inner product, pairwise over tokens), "
    ...
    loss_reduction: str = field(
        ...
        metadata={
            "help": (
                ...
                "samples enter the update weighted by length."
            )
        },
    )
