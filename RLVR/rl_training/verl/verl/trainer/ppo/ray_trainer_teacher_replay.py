...
def select_buffer(buffer, teacher_scores, config):
    ...
    if config.data.replay_strategy == "teacher":
        ...
        # question_index_to_pos = {indices[pos]: pos for pos in range(0,len(buffer),8)}
        # unique_question_indices = [key for key in question_index_to_pos]
        # unique_question_indices = torch.tensor(unique_question_indices).to(teacher_scores.device)
        # buffer_teacher_scores = teacher_scores[unique_question_indices]
        # dataset_sampling_scores = -torch.abs(buffer_teacher_scores - config.data.alpha)
        # # softmax
        # dataset_sampling_logits = dataset_sampling_scores / config.data.tau
        ...
        # selected_indices = torch.multinomial(dataset_sampling_probabilities, int((1-config.data.sigma)*config.data.train_batch_size), replacement=False)
        # selected_question_indices = unique_question_indices[selected_indices]
    ...
    # assert len(selected_data) // 8 == int((1-config.data.sigma)*config.data.train_batch_size), "Selected data length is not correct!"
                            ...
                            with _timer('Rollout_update', epoch_raw):
                                for batch_step, batch_dict in enumerate(tqdm(use_train_dataloader, desc="Rollout_update")):
                                    ...
                                    if epoch == 0:
                                        ...
                                        start_time = time.time()
                                        self.replay_buffer, replay_batch = select_buffer(self.replay_buffer, predicted_labels, self.config) # add here
                                        if self.replay_buffer is None:
                                    ...
                                    if self.config.actor_rollout_ref.actor.use_temp_log_prob:
                                        ...
                                        batch = batch.union(temp_log_prob) # sigma*B, (1-sigma)*B
