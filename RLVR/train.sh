...
reset_config() {
    ...
    # (seq-mean-token-sum-norm = Dr. GRPO); the hook follows whichever mode is set.
    ...
    cfg_refresh_freq="1"
    cfg_hook_embeddings="true"        # hook embed_tokens + tied lm_head (drpt_verl default); "false" = block Linear layers only
    cfg_total_training_steps=""       # cap on training rounds (verl trainer.total_training_steps); empty = epochs only
    cfg_tie_embeddings="false"        # "true": embed_tokens + tied lm_head share one target gradient and one subset (needs hook_embeddings)
    cfg_exp_suffix=""                 # appended to EXP_NAME / output dir to keep variants apart
...
parse_yaml() {
    ...
    while IFS= read -r line; do
        ...
        case "$key" in
            ...
            model)                           cfg_model="${val//\$\{SCRATCH_DIR\}/$SCRATCH_DIR}" ;;   # yaml may write ${SCRATCH_DIR}/...
            seed)                            cfg_seed="$val" ;;
            ...
            hook_embeddings)                 cfg_hook_embeddings="$val" ;;
            tie_embeddings)                  cfg_tie_embeddings="$val" ;;
            total_training_steps)            cfg_total_training_steps="$val" ;;
...
run_method() {
    ...
    if [[ "$selection_enabled" == "True" ]]; then
        EXP_NAME="${model_name}_${cfg_name_method:-$cfg_method}_s${cfg_seed}_${cfg_val_loss_type}"
    else
    ...
    [[ ( "$selection_enabled" == "True" || "$cfg_eval_cleaned_test" == "true" ) && -f "$math_test_cleaned_path" ]] && test_files="$math_test_cleaned_path"
