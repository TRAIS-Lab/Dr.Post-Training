...
def compute_accuracy(
    ...
    predictions = generate_completions(
        ...
        do_sample=False,   # greedy (an earlier version sampled at temperature 1.0 / top_p 0.95)
        disable_tqdm=False
    )
    ...
    try:   # per-example ROUGE-L for the saved generations (diagnostics only)
        _per = rouge_metric.compute(predictions=cleaned_predictions, references=references, use_stemmer=True, use_aggregator=False)["rougeL"]
    except Exception:
        _per = [None] * len(cleaned_predictions)
    dump_generations(args, "samsum", [{"prediction": p, "raw": g, "references": [r], "rougeL": (100 * s if s is not None else None)}
                                      for p, g, r, s in zip(cleaned_predictions, predictions, references, _per)], tokenizer)
