...
class Compressor(ProjectionContainer):
    ...
    def forward(self, input: Union[Tuple[torch.Tensor, torch.Tensor], torch.Tensor]) -> torch.Tensor:
        """
        Apply the full compression pipeline (see :meth:`_forward_ref` for the two modes).

        For the common case — component mode on 3-D CUDA bf16/fp16 activations, dense
        (normal / rademacher) first-stage factors of width <= 64 and an identity second
        stage — the whole pipeline runs as one fused kernel from ``drpt.kernels``
        (``compressed_grad``): both projections and the per-sample outer product are
        formed on-chip, replacing two skinny GEMMs, a bmm and a scaling pass.
        """
        if isinstance(input, tuple):
            fused = self._fused_components(input)
            if fused is not None:
                return fused
        return self._forward_ref(input)

    def _fused_components(self, input: Tuple[torch.Tensor, torch.Tensor]) -> Optional[torch.Tensor]:
        """Fused-kernel path for component mode, or None when not applicable."""
        from .kernels import fused_ok, backend, kernel_ops

        c1, c2 = input
        if backend() != "cute" or not fused_ok(c1, c2, op="proj"):
            return None
        plan = self._fused_plan()
        if plan is None:
            return None
        P1, P2, scale = plan
        if (P1.dtype != c1.dtype or P2.dtype != c2.dtype or P1.shape[0] != c1.shape[-1]
                or P2.shape[0] != c2.shape[-1]):
            return None
    ...
    def _fused_plan(self):
        ...
        identity; cached per compressor and recomputed after a refresh (new projection matrices)."""
        from .projection import ProjectionType

        s1, s2 = self.sparsifier.sparsifier_comp if self.sparsifier is not None else (None, None)
        proj = self.projector.projector if self.projector is not None else None
        P1 = getattr(s1, "proj_matrix", None)
        P2 = getattr(s2, "proj_matrix", None)
        cache = getattr(self, "_fused_plan_cache", None)
        if cache is not None and cache[0] is P1 and cache[1] is P2 and cache[2] is proj:
            return cache[3]
        plan = None
        dense = (ProjectionType.normal, ProjectionType.rademacher)
        if (proj is not None and proj.proj_type == ProjectionType.identity and P1 is not None and P2 is not None
                and s1.proj_type in dense and s2.proj_type in dense and P1.shape[1] <= 64 and P2.shape[1] <= 64):
            P1c = P1 if P1.is_contiguous() else P1.contiguous()
            P2c = P2 if P2.is_contiguous() else P2.contiguous()
            # CudaProjector.project divides each projection by sqrt(proj_dim)
            plan = (P1c, P2c, 1.0 / math.sqrt(s1.proj_dim * s2.proj_dim))
        self._fused_plan_cache = (P1, P2, proj, plan)
        return plan

    @torch.compile
    def _forward_ref(self, input: Union[Tuple[torch.Tensor, torch.Tensor], torch.Tensor]) -> torch.Tensor:
        """
