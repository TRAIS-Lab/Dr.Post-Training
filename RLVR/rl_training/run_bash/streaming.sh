...
VAL_RATIO=0.5                 # Fraction of validation batch to use after difficulty selection (Goldilocks)
TRAIN_SELECTION_RATIO=1.0     # Fraction of negative-influence samples to DROP (filtering mode) - 1.0 = drop ALL negative samples
VAL_SELECTION_MODE="none"     # "soft" (probabilistic) or "hard" (top-k)
USE_SECOND_ORDER=False        # Whether to use second-order greedy selection
SELECTION_VAL_BATCH_SIZE=64   # Batch size for separate validation set (used once per epoch) - reduced to avoid OOM
