...
# =============================================================================
# Embedding layers: lookup-based scoring (the embedding analogue of PIP)
# =============================================================================
#
# The per-sample gradient of an embedding matrix W in R^{V x D} is sparse: row v collects the
# output gradients of the positions whose token is v. Hence
#   <G_b, G_val> = sum_{t in sample b} <grad_output[t], G_val[input_ids[t]]>,
# a table lookup plus a dot product per token, and the curated gradient is a scatter-add of the
# output gradients of the kept samples. Nothing of size [B, V, D] is ever materialized.
# All helpers accept both layouts the actor uses: standard ([B, S, D] with ids [B, S]) and
# packed ([1, total_nnz, D] with ids [1, total_nnz] + cu_seqlens).
...
def compute_embedding_val_gradient(
    ...
    """
    Total embedding gradient of a batch: grad_weight[v] = sum over positions with token v of grad_output.

    Layout-agnostic (only the flattened token list is needed). Mirrors F.embedding's backward,
    including the zeroed padding_idx row.

    Args:
        grad_output: [B, S, D], [1, total_nnz, D] or [B, D]
        input_ids: token ids with the same leading shape as grad_output
        num_embeddings: vocabulary size V
        padding_idx: padding row to zero (-1 for none)
...
def compute_embedding_scores_packed(
    grad_output: Tensor,
    input_ids: Tensor,
    ...
    """
    Per-sample embedding scores from a packed batch:
        score_b = sum_{t in sample b} <grad_output[t], val_grad_weight[input_ids[t]]>.

    Args:
        grad_output: [1, total_nnz, D]
        input_ids: [1, total_nnz] (or [total_nnz])
        val_grad_weight: validation embedding gradient [V, D]
        cu_seqlens: cumulative sequence lengths [batch_size + 1]

    Returns:
        (scores [batch_size], sample_ids [total_nnz]) - sample_ids can be reused for the gradient.
    """
    go = grad_output.reshape(-1, grad_output.shape[-1])          # [total_nnz, D]
    ids = input_ids.reshape(-1)                                   # [total_nnz]
    val = val_grad_weight.to(go.dtype)
    per_token_scores = (go * val[ids]).sum(dim=1)                 # [total_nnz]

    sample_ids = compute_sample_ids_from_cu_seqlens(go.shape[0], cu_seqlens, go.device)
    batch_size = cu_seqlens.shape[0] - 1
    # Accumulate per-sample scores in fp32: under autocast (Global Subset scoring pass) the per-token
    # ``sum`` is promoted to fp32 while ``go`` is bf16, and scatter_add_ requires equal dtypes. The caller casts
    # to its accumulator dtype anyway (GlobalSubsetStateVerl.accumulate_precomputed_scores).
    scores = torch.zeros(batch_size, device=go.device, dtype=torch.float32)
    scores.scatter_add_(0, sample_ids, per_token_scores.float())
    return scores, sample_ids


def compute_embedding_selected_gradient_packed(
    grad_output: Tensor,
    input_ids: Tensor,
    selected_indices: Tensor,
    cu_seqlens: Tensor,
    ...
    """
    Curated embedding gradient from a packed batch: scatter-add of the output gradients of the
    tokens that belong to the selected samples, times scale_factor.

    Returns:
        grad_weight: [V, D]
    """
    dim = grad_output.shape[-1]
    go = grad_output.reshape(-1, dim)
    ids = input_ids.reshape(-1)
    if sample_ids is None:
        sample_ids = compute_sample_ids_from_cu_seqlens(go.shape[0], cu_seqlens, go.device)
    token_mask = torch.isin(sample_ids, selected_indices)

    grad_weight = torch.zeros(num_embeddings, dim, device=go.device, dtype=go.dtype)
    grad_weight.index_add_(0, ids[token_mask], go[token_mask])
...
def compute_embedding_scores_standard(
    grad_output: Tensor,
    input_ids: Tensor,
    val_grad_weight: Tensor,
) -> Tensor:
    """
    Per-sample embedding scores for the standard layout ([B, S, D] with ids [B, S], or [B, D] with ids [B]).
    """
    val = val_grad_weight.to(grad_output.dtype)
    prod = grad_output * val[input_ids]
    return prod.sum(dim=tuple(range(1, prod.dim())))


def compute_embedding_selected_gradient_standard(
    grad_output: Tensor,
    input_ids: Tensor,
    selected_indices: Tensor,
    scale_factor: Tensor,
    ...
    """
    Curated embedding gradient for the standard layout: scatter-add over the selected samples' tokens.
    """
    grad_weight = compute_embedding_val_gradient(
        grad_output[selected_indices], input_ids[selected_indices], num_embeddings, padding_idx
    )
    grad_weight.mul_(scale_factor)
    return grad_weight
