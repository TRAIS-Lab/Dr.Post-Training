...
# Default configs covering both sides of the pip/gip crossover.
...
COMBOS = [
    ("full_training",        "pip",  "none"),
    ("layer_wise_subset",       "compress",       "normal-64*64"),
    ("layer_wise_subset",       "gip",     "none"),
    ("layer_wise_subset",       "pip",  "none"),
    ("layer_wise_subset",       "direct",         "none"),
    ("global_subset",          "compress",       "normal-64*64"),
    ("global_subset",          "gip",     "none"),
    ("global_subset",          "pip",  "none"),
    ("global_subset",          "direct",         "none"),
    ("global_subset_one_pass", "compress",       "normal-64*64"),
    ("global_subset_one_pass", "gip",     "none"),
    ("global_subset_one_pass", "pip",  "none"),
...
def print_summary(all_results, model_name):
    ...
    print(f"  {'Config':<22} {'compress':>10} {'gip':>10} {'pip':>10} {'direct':>10}")
    print(f"  {'-'*62}")
    for label, config_results in all_results.items():
        std_r = config_results.get("full_training/pip")
        std_total = compute_total(std_r, "full_training") if std_r else None
        if std_total is None:
            continue
        vals = []
        for scoring in ["compress", "gip", "pip", "direct"]:
  ...
  python benchmark_run.py --gpu 2 --output results/llama.json
    ...
    parser.add_argument("--only-scoring", type=str, default=None, choices=["compress", "gip", "pip", "direct"],
                        help="Run only full_training and the combos with this scoring method; when --output "
                             "exists, its other entries are kept and these are updated in place")
    args = parser.parse_args()
    combos = [c for c in COMBOS if args.only_scoring is None or c[0] == "full_training" or c[1] == args.only_scoring]
    previous = {}
    if args.only_scoring and args.output and os.path.exists(args.output):
    ...
    for config in configs:
        ...
        config_results = dict(previous.get(label, {}))
