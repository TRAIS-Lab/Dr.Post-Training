...
def run_ppo_online_selection(config):
    ...
    if not ray.is_initialized():
        ...
        # Ray start-up can time out when another Ray instance is tearing down on the same node: retry a few times.
        ...
        for _attempt in range(1, 4):
            try:
                ...
                if _attempt == 3:
                    raise
                print(f"[Selection] ray.init failed (attempt {_attempt}/3): {_e}; retrying in 90 s", flush=True)
                try:
                    ray.shutdown()
