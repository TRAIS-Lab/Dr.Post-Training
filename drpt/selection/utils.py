...
def compute_total_gradient(
    ...
    if grad_output.dim() == 3:
        if fused_ok(grad_output, input, op="wgrad"):
            # Fused Ampere GEMM over all B*S tokens (drpt/kernels); same fp32 accumulation
            # and output dtype as the einsum, ~20 % faster on A40 for K = B*S >~ 2k.
            return kernel_ops().total_wgrad(grad_output, input)
...
def compute_scores_and_similarity(
    ...
    """
    Per-token inner product (PIP) scoring.

    Dispatches to the fused kernel of the active backend (``drpt.kernels``: CuTe DSL by
    default, Triton as fallback) for 3-D CUDA half-precision inputs — the ``[B, S, O]``
    intermediate is never materialised and scores accumulate in fp32 — and otherwise to
    the ``torch.compile``d reference :func:`_compute_scores_and_similarity_ref`.
    See the reference for the math and complexity.
    """
    if fused_ok(train_grad_output, train_input, op="pip"):
        pip_scores = kernel_ops().pip_scores
        ...
        if val_grad_total is None:
            ...
            val_grad_total = compute_total_gradient(
                val_grad_output.to(target_dtype), val_input.to(target_dtype)
            )
        scores = pip_scores(train_grad_output, train_input, val_grad_total.to(target_dtype))
        similarity = _train_similarity_3d(train_grad_output, train_input) if use_second_order else None
        return scores, similarity
    return _compute_scores_and_similarity_ref(
        train_grad_output, train_input, val_grad_output, val_input, val_grad_total, use_second_order
    )


def _train_similarity_3d(train_grad_output: Tensor, train_input: Tensor) -> Tensor:
    """<g_i, g_j> for 3-D data via the contracted per-sample gradients [B, O*I]."""
    contracted = torch.bmm(train_grad_output.permute(0, 2, 1), train_input).flatten(start_dim=1)
    return torch.matmul(contracted, contracted.T)


@torch.compile
def _compute_scores_and_similarity_ref(
    train_grad_output: Tensor,
    train_input: Tensor,
    val_grad_output: Optional[Tensor],
    val_input: Optional[Tensor],
    ...
    """
    Per-token inner product (PIP) scoring via the collapse-first approach.
    ...
    over all train samples. For small V (m=1-2), gip may be faster
...
def compute_scores_gip(
    ...
    """
    Ghost inner product (GIP) scoring, GREATS-style (true ghost, no materialization).
    ...
    PIP (precompute the total target gradient, then score).
    ...
    if train_grad_output.dim() == 3:
        if val_grad_output is not None and val_input is not None:
            ...
            if fused_ok(train_grad_output, train_input, val_grad_output, val_input, op="gip"):
                # Fused kernel (drpt.kernels): both dot-product tiles are formed and reduced
                # in registers, so the [B, V, S, S] pair below is never materialised.
                scores = kernel_ops().gip_scores(train_grad_output, train_input, val_grad_output, val_input)
            else:
            ...
            if fused_ok(train_grad_output, train_input, op="pip"):
                scores = kernel_ops().pip_scores(train_grad_output, train_input, val_grad_total)
            else:
...
def compute_selected_gradients(
    ...
    """
    if fused_ok(train_grad_output, train_input, op="wgrad") and selected_indices.is_cuda:
        # Fused kernel (drpt.kernels): walks the selected samples through the index
        # list (no gathered copies) and applies scale_factor / the bias gradient in
        # the same pass.
...
def compute_embedding_scores(
    ...
    This is the embedding analogue of the per-token inner product (PIP) — we
