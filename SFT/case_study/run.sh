...
# Defaults — match `less_tydiqa` from the main 4-setting matrix. Override via CLI to run any of the other
# 3 settings, or use --sweep to run all 4 settings x 5 seeds. As a Slurm array: SFT/train/qa_matrix_manifest.py writes
# manifests/case_study.txt (one line of run.sh flags per pending run) for SFT/train/slurm/qa_array.sbatch (task script SFT/case_study/run.sh).
# Figures: python SFT/case_study/make_figures.py (reads the selection_records.json of every run).
...
# finished run (Slurm array re-submission): nothing to do
[[ "$sweep" == "true" ]] || { _d="$runs_root/case_study/${train_dataset}_${task}-$(basename "$model")-p${percentage}-lr${lr}-b${batch_size}-v${n_val}-s${seed}"; [[ -s "$_d/selection_records.json" ]] && { echo "[case_study] records exist, skipping: $_d"; exit 0; }; }
    ...
    for entry in "${SETTINGS[@]}"; do
        ...
        for s in "${SEEDS[@]}"; do
            ...
            bash "$SCRIPT" --train "$tr" --task "$tk" --percentage "$pct" \
                --seed "$s" --lr "$lr" --runs_root "$runs_root" $dry_flag
        done
    done
    exit 0
fi
...
# Case-study dirs live under <runs root>/case_study/ (sibling of the main and Target-Only run dirs; the inner name has no
# "case_study_" prefix since the parent dir encodes it, and must not collide with the main runs of the same {train}_{task} prefix).
JOB_NAME="${train_dataset}_${task}-${model_name}-p${percentage}-lr${lr}-b${batch_size}-v${n_val}-s${seed}"
output_dir=$runs_root/case_study/${JOB_NAME}
mkdir -p "$output_dir"
