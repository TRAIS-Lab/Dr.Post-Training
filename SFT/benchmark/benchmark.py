...
def measure_layer_wise_subset(model, optimizer, grad_hook, train_batches, val_batches, config, tokenizer):
    ...
    from drpt.selection.utils import compute_selected_gradients
    ...
    def timed_compressed(hm, sc, uc, state, lidx, inp, go, bias, cvm, usv):
        ...
        if _bwd._FUSED_SELECT and not cvm and state is not None and not usv and uc is None:
            ...
            if parts is not None and state.selection_mode == "topk" and not state.use_second_order \
                ...
                gw, gb = compute_selected_gradients(tgo, ti, si, hb, scale_factor)
                rec.mark('wgrad')
                return gw, gb
        ...
        if uc is not None:
            sel_go = tgo[si]
            sel_inp_aug = augment_input_for_bias(ti[si], hb)
            ...
            # Same call as the library path (fused Triton kernel or gather+einsum reference)
            gw, gb = compute_selected_gradients(tgo, ti, si, hb, scale_factor)
            rec.mark('wgrad')
            return gw, gb
    ...
    def timed_full(hm, uc, state, lidx, inp, bias, go, cvm, usv):
        ...
        sm = getattr(state, 'scoring_method', 'pip')
        fused = _bwd.exact_scores_fused(state, sm, tgo, ti, vgo, vinp, vgt, hb, select=True)
        if fused is not None:
            scores, si = fused
            rec.mark('score')
            rec.mark('select'); _bwd._record_selection(state, lidx, si, scores); rec.mark('select')
            rec.mark('wgrad')
            gw, gb = _produce_gu(hm, uc, state, lidx, tgo, ti, si, hb, materialized_grads=None)
            rec.mark('wgrad')
            return gw, gb
...
def print_results(full_training, layer_wise_subset, global_subset, config, peak_mem, global_subset_one_pass=None):
    ...
    scoring = getattr(config, 'scoring_method', 'pip')
...
def run_method(method, config):
    ...
    from drpt.kernels import set_backend, backend, fused_kernels_enabled
    set_backend("off" if (not config.fused or config.kernel_backend == "off") else config.kernel_backend)
    print(f"  Fused kernels: {backend()}")
    if config.cublaslt:
        # cuBLASLt heuristics avoid the 3.5x-slow cuBLAS kernel that torch 2.6/cu124 picks for the
        # Llama-3.2-3B down_proj weight gradient ([3072, 8192] output, K in 2048..4608) on A40.
        torch.backends.cuda.preferred_blas_library("cublaslt")
        print("  BLAS: cuBLASLt preferred")
...
def main():
    ...
    parser.add_argument('--scoring-method', type=str, default=None,
                        choices=['pip', 'gip', 'direct', 'compress'])
    ...
    parser.add_argument('--no-fused', action='store_true',
                        help='Disable the fused kernels (drpt.kernels); use the PyTorch reference ops.')
    ...
    if not config.fused:
        cli_args += ["--no-fused"]
    elif config.kernel_backend != d.kernel_backend:
        cli_args += ["--kernel-backend", config.kernel_backend]
    if config.cublaslt:
        cli_args += ["--cublaslt"]
