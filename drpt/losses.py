...
``"token_mean"`` (the Hugging Face default)
...
# Upper bound on the float32 logits handled by one cross-entropy call. Larger batches are
# processed a few rows at a time (memory only; every per-token value is computed exactly as
# before). Default 4 GiB: Qwen3-1.7B at 9 x 2048 tokens (11.2 GB of fp32 logits) is chunked so
# the merged batch fits a 44 GB card, Llama-3.2-1B at 9 x 512 tokens (2.4 GB) is not.
CE_CHUNK_BYTES = int(float(os.environ.get("DRPT_CE_CHUNK_BYTES", 4 * 2**30)))


def _shifted_per_token_ce(logits: "Tensor", shift_labels: "Tensor", ignore_index: int) -> "Tensor":
    """[b, S-1] next-token cross-entropy of ``logits`` [b, S, V] against the already shifted labels."""
...
def causal_lm_loss(
    ...
    Args:
        ...
        chunk_rows: rows per cross-entropy call. None picks the largest count whose float32
            logits stay under ``CE_CHUNK_BYTES`` (``DRPT_CE_CHUNK_BYTES``); the result does not
            depend on it.
    """
    validate_loss_reduction(reduction)
    ...
    if chunk_rows is None:
        row_bytes = seq * logits.shape[-1] * 4
        chunk_rows = batch if batch * row_bytes <= CE_CHUNK_BYTES else max(1, CE_CHUNK_BYTES // row_bytes)
    if chunk_rows >= batch:
        per_token = _shifted_per_token_ce(logits, shift_labels, ignore_index)
    else:
        per_token = torch.cat([
            _shifted_per_token_ce(logits[i:i + chunk_rows], shift_labels[i:i + chunk_rows], ignore_index)
            for i in range(0, batch, chunk_rows)
        ], dim=0)
