...
# --gpu must take effect before torch / the CuTe DSL initialise the CUDA driver (they
# read CUDA_VISIBLE_DEVICES once, at import); setting it inside main() is too late.
import os
import sys
if "--gpu" in sys.argv:
...
def compute_thresholds(d):
    ...
    tstar = sum_OI / sum_sqrtOI     # T where pip == direct
...
def estimate_gip_memory(n, T, m, O):
    """Estimate peak memory for gip go_dot tensor in bytes."""
...
def time_scoring_one_layer(method, n, T, m, O, I, device,
    ...
    # Warmup (includes torch.compile JIT for pip)
...
METHODS = ["compress", "gip", "pip", "direct"]
...
def main():
    ...
    print(f"{'='*80}")
    print(f"\n  {'Config':<22s}  {'compress':>10s}  {'gip':>10s}  {'pip':>10s}  {'direct':>10s}  {'Best exact':>12s}")
    print(f"  {'-'*78}")
    ...
    for label, r in all_results.items():
        ...
        exact = {m: r[m]["total_ms"] for m in ["gip", "pip", "direct"]
