...
def main():
    ...
    if has_score_compression or has_update_compression:
        ...
        if has_update_compression:
            ...
            # The tied embedding / output-head weight (Llama ties embed_tokens and lm_head) is one Parameter hooked as two
            # layers. Compressing lm_head's update through the per-layer sketch would (a) update the 128k x d matrix from a
            # 512x512 sketch of the output-head gradient alone and (b) drop the input-embedding gradient, because the
            # optimizer keys parameters by identity and takes the compressed path when a compressed grad exists (the
            # symptom: models reach the expected perplexity but emit EOS / the user marker as the first token).
            # Embeddings and any layer sharing their weight therefore keep the standard (uncompressed) AdamW update.
            tied_ids = {id(model.get_input_embeddings().weight)}
            if model.get_output_embeddings() is not None:
                tied_ids.add(id(model.get_output_embeddings().weight))
            for _idx, _name in enumerate(layer_names):
                _mod = model.get_submodule(_name)
                if update_compressors[_idx] is not None and id(getattr(_mod, "weight", None)) in tied_ids:
                    update_compressors[_idx] = None
                    logger.info(f"  Update compression: excluded {_name} (tied embedding/output weight -> standard AdamW)")
