...
tests/test_loss_convention.py for the convention itself and the alternative "token_mean".
...
# so the "sample_mean" and "token_mean" conventions give different scores.
...
def hf_loss_of(model, batch):
    """Hugging Face's token mean over the batch ("token_mean" convention)."""
...
def run_separate(method, frac=FRAC, granularity="block", rules=None, record=True):
    ...
    strategy = create_separate_batch_strategy(
        ...
        record_selections=record, scoring_method="pip", subset_mode="one_pass",
    )
