#!/bin/bash
# train_v3.sh = train_v2.sh + `--runs_root <dir>` (run dirs land under <dir> instead of $SCRATCH_DIR/.../SFT/runs) and the
# `loss_reduction` yaml key (sample_mean | token_mean, forwarded as --loss_reduction; default = train.py default sample_mean).
# 2026-09-11, for the sample-mean-convention grid (runs_v2). Versioned copy: never edit a launcher while jobs run it.
# train_v2.sh = train.sh + the init_eot_from_eos yaml key (2026-09-10). Versioned copy: never edit train.sh while jobs run it.
#
# SFT Training Runner
#
...
Optional:
  ...
  --runs_root <dir>       Parent directory for run dirs (default $SCRATCH_DIR/Dr.Post-Training/SFT/runs)
...
run_method() {
    ...
    local runs_root="${runs_root_override:-$SCRATCH_DIR/Dr.Post-Training/SFT/runs}"
