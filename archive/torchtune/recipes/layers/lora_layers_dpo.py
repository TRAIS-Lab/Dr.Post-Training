...
class LoRALinear(nn.Linear, LoRALayer):
    ...
    def __init__(
        ...
        **kwargs
        ...
        if r > 0:
            ...
            self.lora_A = nn.Parameter(torch.zeros(r, in_features, dtype=torch.bfloat16, device=self.weight.device))
            self.lora_B = nn.Parameter(torch.zeros(out_features, r, dtype=torch.bfloat16, device=self.weight.device))
    ...
    def forward(self, x: torch.Tensor):
        # print(self.r, self.merged, self.disabled)
        def T(w):
            return w.transpose(0, 1) if self.fan_in_fan_out else w
        if self.disabled:
            if self.merged:
                weight = self.weight - T(self.lora_B @ self.lora_A) * self.scaling
                # print("merged", weight[0][0:5])
                return  F.linear(x, T(weight), bias=self.bias)
            else:
                # print("not merged", self.weight[0][0:5])
...
class GCLoRALinear(LoRALinear):
    def __init__(self, in_features, out_features, r=0, lora_alpha=1, lora_dropout=0., 
                 fan_in_fan_out=False, merge_weights=True, disabled=False, **kwargs):
        super(GCLoRALinear, self).__init__(in_features, out_features, r=r, lora_alpha=lora_alpha,
                                           ...
                                           merge_weights=merge_weights, disabled=disabled, **kwargs)
