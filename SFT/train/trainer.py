...
class LayerWiseSubsetTrainer(Trainer):
    ...
    def __init__(self, grad_hook: GradientHook, val_dataset, *args, **kwargs):
        ...
        # Opt-in kernel-level profiling of a window of real steps (SFT/benchmark/trace_attribution.py):
        #   DRPT_PROFILE_STEPS=30:40 DRPT_PROFILE_TRACE=<dir>/trace.json  -> <dir>/profile.json
        if os.environ.get("DRPT_PROFILE_STEPS"):
            from SFT.benchmark.trace_attribution import StepProfilerCallback
            self.add_callback(StepProfilerCallback(
                self, os.environ["DRPT_PROFILE_STEPS"],
                os.environ.get("DRPT_PROFILE_TRACE", os.path.join(self.args.output_dir, "trace.json")),
                keep_trace=os.environ.get("DRPT_PROFILE_KEEP_TRACE") == "1"))
        ...
        if val_strategy == 'merged_batch':
            self.selection_strategy = create_merged_batch_strategy(
                ...
                score_normalization=getattr(self.args, 'score_normalization', 'none'),
            )
        else:
    ...
    def training_step(self, model, inputs, num_items_in_batch=None):
        ...
        if args.method in SELECTION_METHODS:
            ...
            if self.val_strategy == 'merged_batch':
                ...
                #   gip   → factorized [V,S,O] + [V,S,I] (for pairwise scoring)
                #   pip/direct → full [O,I] (summed gradient, cheaper)
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        ``"token_mean"``: Hugging Face's default token mean over the batch (every supervised token is one loss item).
        """
