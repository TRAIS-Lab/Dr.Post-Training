...
def get_trainable_linear_layers(
    model: nn.Module,
    skip_grad_check: bool = False,
    hook_embeddings: bool = True,
) -> List[str]:
    """
    Get names of the modules to hook for data selection.

    With ``hook_embeddings=True`` (default) this is every trainable ``nn.Linear`` (the transformer
    blocks and the output layer ``lm_head``) and every trainable ``nn.Embedding`` (the token
    embedding, scored with the lookup-based rule in ``packed_ops``), i.e., the same coverage as the
    SFT hook: all trainable parameters are then either hooked or normalization weights.
    ``hook_embeddings=False`` hooks only the block-internal
...
class DataParallelPPOActorWithSelection(DataParallelPPOActor):
    ...
    def _initialize_grad_hook(self) -> None:
        ...
        # Get trainable layer names (Linear + Embedding unless hook_embeddings=False)
        layer_names = get_trainable_linear_layers(
            unwrapped_module, skip_grad_check=is_fsdp, hook_embeddings=self.hook_embeddings
        )

        if len(layer_names) == 0:
            logger.warning("No trainable Linear/Embedding layers found, falling back to NA selection")
        ...
        logger.info(
            ...
            f"(hook_embeddings={self.hook_embeddings})"
        )
    ...
    def capture_validation_gradients_external(
        ...
        Supports two validation loss types:
            ...
            - "nice": L = -E[reward * log_prob] with the RAW reward (NICE, Wang et al. ICML 2025): the plain
                Monte-Carlo policy gradient of the metric, no baseline and no normalization; with 0/1 verifier rewards
                only the correct responses contribute.
        ...
        if val_loss_type == "reward":
        ...
        elif val_loss_type == "nice":
            # NICE objective: -E_{y~pi}[ r(x, y) * grad log pi(y|x) ] estimated by Monte Carlo over the
            # n_responses samples per prompt, with the raw (unnormalized, un-centered) metric as the weight.
            # Sequence aggregation and the global item count are shared with "reward"; a positive
            # rescaling does not change zero-thresholded selection, the missing baseline does.
            seq_scores = val_rewards.to(device=val_input_ids.device, dtype=torch.float32)
            norm_stats.update({'used_fallback': False, 'zero_var_groups': 0, 'total_groups': 0, 'norm_type': 'none'})
            norm_stats['num_samples_used'] = batch_size
            logger.info(
                f"[Val Grad] Using NICE raw-reward policy gradient on {batch_size} samples "
                f"(mean reward {seq_scores.mean().item():.3f}, n_responses={n_responses})"
            )
            ...
            raise ValueError(
                ...
                f"Supported: reward, train-loss, nice"
            )
        ...
        stats = {
            ...
            'val/tied_pairs': len(getattr(self.grad_hook, 'tied_pairs', {})) // 2,
    ...
    def _setup_layer_wise_subset(
        ...
        # No per-micro-batch logging (too verbose)
            ...
            for batch_idx, mini_batch in enumerate(mini_batches):
                ...
                for micro_batch in micro_batches_list:
                    ...
                    try:
                        ...
                        if self.keep_zero_adv and self.grad_hook.selection_state is not None:
                            rm = response_mask.to(advantages.dtype)
                            adv_per_sample = (advantages * rm).sum(dim=1) / rm.sum(dim=1).clamp(min=1)
                            self.grad_hook.selection_state.force_keep = adv_per_sample.abs() < 1e-8
