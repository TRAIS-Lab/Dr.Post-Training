#!/bin/bash
# Submit benchmark jobs to Slurm (one A40 job per benchmark; the full paper suite is a40_paper_suite.sbatch).
#
# Benchmarks:
...
ACCOUNT="${SLURM_ACCOUNT:-my-account}"
PARTITION="${SLURM_PARTITION:-gpu}"
CONDA_ENV="${CONDA_ENV:-${CONDA_PREFIX:?activate the conda env or set CONDA_ENV}/bin}"
...
for dir in breakdown breakdown_checkpointing; do
    ...
    for scoring in compress gip pip direct; do
