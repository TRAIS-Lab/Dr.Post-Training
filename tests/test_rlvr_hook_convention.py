...
4. selection_frac=1 top-k reproduces the full seq-mean-token-mean gradient; verl's default
...
def test_legacy_target_is_token_weighted():
    ...
    close(cached.float(), h_w["2.weight"].float(), "token-mean token-weighted target", atol=1e-6, rtol=1e-4)
