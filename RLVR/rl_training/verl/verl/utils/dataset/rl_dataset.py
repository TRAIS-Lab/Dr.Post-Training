...
class RLHFDataset(Dataset):
    ...
    def _read_files_and_tokenize(self):
        ...
        if not self.format_reward:
            self.dataframe = self.dataframe[self.dataframe.apply(lambda doc: len(
                tokenizer.apply_chat_template(doc[prompt_key], add_generation_prompt=True)) <= self.max_prompt_length,
                                                                axis=1)]
        else:
            self.dataframe = self.dataframe[self.dataframe.apply(lambda doc: len(
                tokenizer.apply_chat_template(doc[prompt_key], continue_final_message=True)) <= self.max_prompt_length,
                                                                axis=1)]
    ...
    def __getitem__(self, item):
        ...
        if not self.use_question:
            ...
            input_ids, attention_mask = verl_F.tokenize_and_postprocess_data(prompt=question,
                                                                            tokenizer=self.tokenizer,
                                                                            max_length=self.max_prompt_length,
                                                                            pad_token_id=self.tokenizer.pad_token_id,
                                                                            left_pad=True,
