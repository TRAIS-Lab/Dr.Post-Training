...
MODE="${1:-both}"
# Where results go (default: SFT/benchmark/results). RESULTS_ROOT=SFT/benchmark/results/paper keeps a
# fresh run separate; EXTRA_ARGS is appended to every benchmark_run.py call (e.g. --kernel-backend triton).
RESULTS_ROOT="${RESULTS_ROOT:-SFT/benchmark/results}"
EXTRA_ARGS="${EXTRA_ARGS:-}"

# ── Benchmark 1: Breakdown ──────────────────────────────────────────────────

run_breakdown() {
    local RESULTS="$RESULTS_ROOT/breakdown"
    mkdir -p "$RESULTS"
    ...
    run_model() {
        ...
        for cfg in "${configs[@]}"; do
            ...
            $PYTHON SFT/benchmark/benchmark_run.py \
                ...
                --direct-batch-size 1 $EXTRA_ARGS \
...
run_breakdown_checkpointing() {
    local RESULTS="$RESULTS_ROOT/breakdown_checkpointing"
    mkdir -p "$RESULTS"
...
run_scoring() {
    local RESULTS="$RESULTS_ROOT/scoring"
    mkdir -p "$RESULTS"
