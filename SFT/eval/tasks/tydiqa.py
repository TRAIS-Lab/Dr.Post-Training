...
from ..utils import generate_completions, get_eos_token_ids, dump_generations
...
def compute_accuracy(args, model, tokenizer):
    ...
    # Batched greedy generation (an earlier version used batch_size=1, ~12 min per run): left padding + greedy makes the batch size immaterial
    # up to bf16 numerics; capped at 16 because the one-shot prompts are long. Recorded in the result as generation_batch_size.
    gen_batch_size = min(int(getattr(args, "batch_size", 1) or 1), 16)
    generations = generate_completions(
        ...
        do_sample=False,   # greedy (an earlier version passed nothing and inherited the base model's generation_config: sampling at T=0.6 / top_p 0.9)
    )
    ...
    for i, (_, answer, lang) in enumerate(test_dataset):
        ...
        _rows.append({"prediction": clean_output, "raw": generations[i], "references": [clean_answer], "lang": lang,
                      "f1": by_lang[lang]["f1"][-1], "em": by_lang[lang]["em"][-1]})

    dump_generations(args, "tydiqa", _rows, tokenizer)
    if not by_lang:
