...
def _dispatch_scoring(
    ...
    elif scoring_method == "gip":
        return (*compute_scores_gip(
            train_grad_output, train_input, val_grad_output, val_input,
...
import os as _os
# DRPT_FUSED_SELECT=0 keeps the fused compress kernel but runs scoring/selection with the reference ops (A/B switch)
_FUSED_SELECT = _os.environ.get("DRPT_FUSED_SELECT", "1").strip().lower() not in ("0", "false", "off", "no")


def compressed_scores_fused(
    hook_manager: "GradientHook",
    ...
    """
    if (not _FUSED_SELECT or state is None or has_bias or hook_manager.capture_val_mode
            or getattr(state, '_use_stored_val', False) or state.use_second_order
            or hook_manager.update_compressors[score_compressor.index] is not None):
        return None
    k = 0
    if select:
        if state.selection_mode != "topk":
            return None
        k = min(state.num_selected, state.train_batch_size)
        if k < 1:
            return None
...
def exact_scores_fused(
    ...
    """pip / gip scoring with the post-GEMM chain fused (CuTe backend).

    The score kernel writes per-CTA partial sums; ``reduce_select`` turns them (plus the bias-gradient
    term when the layer has a bias) into ``corr * scores`` and, for LayerWiseSubset, the sorted top-k —
    replacing the partial sum, the correction multiply, top-k and sort.  None when not applicable.
    """
    from ..kernels import backend, fused_ok, kernel_ops
    if (not _FUSED_SELECT or state is None or backend() != "cute" or state.use_second_order
            or scoring_method not in ("pip", "gip") or train_grad_output.dim() != 3
            or train_grad_output.shape[0] > 256):
        return None
    k = 0
    if select:
        if state.selection_mode != "topk":
            return None
        k = min(state.num_selected, train_grad_output.shape[0])
        if k < 1:
            return None
    ops = kernel_ops()
    dt = train_grad_output.dtype
    if scoring_method == "gip" and val_grad_output is not None and val_input is not None:
        if not fused_ok(train_grad_output, train_input, val_grad_output, val_input, op="gip"):
            return None
        partials = ops.gip_partials(train_grad_output, train_input, val_grad_output.to(dt), val_input.to(dt))
    else:
        if not fused_ok(train_grad_output, train_input, op="pip"):
            return None
        if val_grad_total is None:
            if val_grad_output is None or val_input is None:
                return None
            val_grad_total = compute_total_gradient(val_grad_output.to(dt), val_input.to(dt))
        partials = ops.pip_partials(train_grad_output, train_input, val_grad_total.to(dt))
    if has_bias:
        # bias gradient term <sum_s go_b, sum_v sum_s go_v> as one more column of the partial matrix
        train_bias = train_grad_output.sum(dim=1)
        if val_grad_output is not None:
            val_bias = val_grad_output.sum(dim=(0, 1))
        else:
            return None
        partials = torch.cat([partials, (train_bias @ val_bias.to(train_bias.dtype)).float().unsqueeze(1)], dim=1)
    corr = _correction_f32(state, train_grad_output.device)
    scores, sel = ops.reduce_select(partials, corr, k)
    return scores, (sel if select else None)


def _correction_f32(state, device) -> Tensor:
    """Merged-batch score correction as a 1-element fp32 CUDA tensor (cached per step; never a host read)."""
    c = getattr(state, "score_correction_f32", None)
    if c is None:
        sc = state.score_correction
        if sc is None:
            c = torch.ones(1, dtype=torch.float32, device=device)
        else:
            c = sc.to(torch.float32).reshape(1)
        state.score_correction_f32 = c
    return c


def _record_selection(state, layer_idx: int, selected_indices: Tensor, scores: Tensor) -> None:
    """Bookkeeping shared by the reference and fused selection paths."""
...
class LayerWiseSubsetLinearBackward(Function):
    ...
    def backward(
        ...
        if score_compressor is not None:
            if capture_val_mode:
                ...
                use_compressed_scoring = getattr(state, 'scoring_method', 'pip') == 'compress'
    ...
    def _backward_compressed(
        ...
        # --- Fused compress + score + select (CuTe backend) ---
        fused = compressed_scores_fused(hook_manager, score_compressor, state, grad_output, input, has_bias, select=True)
        if fused is not None:
            scores, selected_indices = fused
            _record_selection(state, layer_idx, selected_indices, scores)
        ...
        if use_stored_val:
            ...
            val_grad = val_grads[0] if val_grads.shape[0] == 1 else val_grads.sum(dim=0)
    ...
    def _backward_full(
        ...
        scoring_method = getattr(state, 'scoring_method', 'pip')
        fused = exact_scores_fused(state, scoring_method, train_grad_output, train_input,
                                   val_grad_output, val_input, val_grad_total, has_bias, select=True)
        if fused is not None:
            scores, selected_indices = fused
            _record_selection(state, layer_idx, selected_indices, scores)
            return _produce_gradient_update(
...
class GlobalSubsetLinearBackward(Function):
    ...
    def backward(
        ...
        use_compressed_scoring = (
            ...
            and getattr(state, 'scoring_method', 'pip') == 'compress'
        )
    ...
    def _accumulate_compressed(
        ...
        fused = compressed_scores_fused(hook_manager, compressor, state, grad_output, input, bias is not None, select=False)
        if fused is not None:
            # scores already carry the merged-batch correction
            state.accumulate_precomputed_scores(fused[0], None, None, layer_idx=layer_idx)
            return
    ...
    def _accumulate_full(
        ...
        fused = exact_scores_fused(state, state.scoring_method, train_grad_output, train_input,
                                   val_go, val_inp, val_grad_total, has_bias, select=False)
        if fused is not None:
            state.accumulate_precomputed_scores(fused[0], None, None, layer_idx=layer_idx)
            return
    ...
    def backward(
        ...
        if train_bs is not None and train_bs < grad_output.shape[0]:
            ...
            if (state is not None
                ...
                # A batch whose training rows carry no supervised token has train_total == 0; the
                # train-slice gradient is then exactly zero and must stay zero (0 * inf would be NaN).
                train_total = state.train_total_tokens_tensor
                scale = torch.where(
                    train_total > 0,
                    state.batch_total_tokens_tensor / train_total.clamp(min=1),
                    torch.ones_like(train_total),
                )
...
class GlobalSubsetEmbeddingBackward(Function):
    ...
    (the embedding analogue of the per-token inner product, PIP).
