...
| `LayerWiseSubset-LoRA.yaml`   | LayerWiseSubset | Per-layer curation with exact (`pip`) scoring |
...
- **IIF**: Iterative Influence-based Filtering — pre-filter entire rollout *before* PPO epochs
...
every response is one item, the target is the mean over validation
...
`loss_reduction=token_mean` selects the alternative convention (token mean over the micro-batch
...
- **Training prompts**: `allenai/real-toxicity-prompts`, prompt toxicity > 0.3, first 80% of the
  filtered rows (unshuffled split), 10–15-token prefixes of prompt+continuation.
- **Held-out 20%** of the filtered rows is split in two halves: the first half supplies the fixed
  ...
  evaluation prompts are therefore disjoint.
...
## Re-scoring the final adapters under several judges

`RLHF/eval/rescore_final.py --run_dir <run> --out <run>/rescore_final.json` generates on the 500 evaluation
prompts with the paper's decoding and scores them with the DaNLP judge of the paper plus three English judges
(LFTW, s-nlp, toxic-bert). `RLHF/eval/rescore_task.sh <BASE | run dir name>` wraps it as a Slurm array task
(one line of `rescore_manifest.txt` per task). `RLHF/tables/scoring_backend.py` turns the json files into the
paper's exact-vs-compressed table; `RLHF/result.ipynb` plots the judge figure. Like `SFT/result.ipynb`, the
notebook writes every paper figure straight into `Paper/ICLR/Figures/RLHF/` (`paper_root()` of `tables/common.py`; the
compressed judge panels, which are not in the paper, go to `RLHF/figures/`).
