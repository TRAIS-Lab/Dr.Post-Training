...
parse_yaml() {
    ...
    while IFS= read -r line; do
        ...
        case "$key" in
            ...
            # Alias keys (nested compression settings)
...
run_method() {
    ...
    # Compressed-scoring runs get their own run directory; exact-scoring runs use the plain names.
    if [[ "$internal_method" != "NA" && -n "$cfg_score_compression" && "$cfg_score_compression" != "none" ]]; then
        local _cmp="${cfg_score_compression%%/*}"; method_str="${method_str}-cmp-${_cmp//\*/x}"
    fi
