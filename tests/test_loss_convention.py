...
5. token_mean. GradientHook(loss_reduction="token_mean") with the HF token-mean loss still
   ...
   paper's score times the sample's token count (a length bias).
...
def run_merged(method, frac=FRAC, subset_mode="one_pass", use_second_order=False,
    ...
    strategy = create_merged_batch_strategy(
        ...
        selection_mode="topk", record_selections=True, scoring_method="pip",
...
def run_separate(method, frac=FRAC, subset_mode="one_pass", use_second_order=False,
    ...
    hook.start_val_capture(scoring_method="pip")
    model.zero_grad()
...
def test_legacy_token_mean_convention_pinned():
    ...
    token-SUM gradient — the paper's score times T_b, i.e. a length bias that "sample_mean" avoids.
    """
    r = ref()
    ...
    for mode, run, tol in [("merged", run_merged, MERGED_TOL), ("separate", run_separate, SEP_TOL)]:
        ...
        worst = tg.assert_same_grads(out["grads"], plain_hf, what=f"token_mean/{mode}/frac=1", **tol)
        print(f"  token_mean {mode:8s} frac=1.0 == HF full training, max|diff|={worst:.2e}")
    ...
    assert_close(scores, legacy_ref.float(), "token_mean merged scores", atol=1e-6, rtol=1e-3)
    # The same identity written with per-example gradients: score_b = (T_b / T_train) *
    ...
    # token_mean target H / T_val is itself token-weighted, unlike the paper's (1/m) sum_j grad lbar_j.
    ...
    assert_close(scores.double(), per_example_vs_hw * t_train / t_train.sum(),
                 "token_mean score == per-example score x T_b / T_train", atol=1e-9, rtol=1e-3)
    paper_sel = sorted(topk_selection(r.scores * LR, K).tolist())
    print(f"  token_mean scores == <G_b, H>/(T_train T_val) == <grad lbar_b, H/T_val> x T_b/T_train; "
          f"tokens/sample {t_train.long().tolist()}; token_mean top-k {selected} vs paper top-k {paper_sel}"
...
def test_chunked_causal_lm_loss_matches_unchunked():
    """Row-chunked cross-entropy (large-vocab memory path) gives the same loss and logits gradient."""
    model = tg.make_model()
    logits = model(input_ids=TRAIN["input_ids"], attention_mask=TRAIN["attention_mask"]).logits.detach()
    for reduction in ("sample_mean", "token_mean"):
        ref_in = logits.clone().requires_grad_(True)
        ref = causal_lm_loss(ref_in, TRAIN["labels"], reduction, chunk_rows=N_TRAIN)
        ref.backward()
        for rows in (1, 2, N_TRAIN - 1):
            x = logits.clone().requires_grad_(True)
            out = causal_lm_loss(x, TRAIN["labels"], reduction, chunk_rows=rows)
            out.backward()
            assert torch.allclose(out, ref, atol=1e-7, rtol=1e-6), f"{reduction} chunk_rows={rows}: {out.item()} vs {ref.item()}"
            assert torch.allclose(x.grad, ref_in.grad, atol=1e-7, rtol=1e-5), f"{reduction} chunk_rows={rows}: logits gradient differs"
    # the automatic threshold: tiny logits are never chunked, a 1-byte budget chunks one row at a time
    import drpt.losses as L
    saved = L.CE_CHUNK_BYTES
    try:
        L.CE_CHUNK_BYTES = 1
        auto = causal_lm_loss(logits, TRAIN["labels"], "sample_mean")
    finally:
        L.CE_CHUNK_BYTES = saved
    assert torch.allclose(auto, causal_lm_loss(logits, TRAIN["labels"], "sample_mean"), atol=1e-7)
    print("  chunked cross-entropy == unchunked (loss and logits gradient) for both reductions")


ALL_TESTS = [
    test_loss_helpers,
    test_chunked_causal_lm_loss_matches_unchunked,
