...
class PreferenceDataset(Dataset):
    ...
    def _prepare_sample(self, sample: Mapping[str, Any]) -> Dict[str, List[int]]:
        ...
        chosen_labels = [CROSS_ENTROPY_IGNORE_IDX if m else t for m, t in zip(c_masks, chosen_input_ids)]

        rejected_input_ids, r_masks = self._tokenizer.tokenize_messages(
            rejected_message, self.max_seq_len
        )
        rejected_labels = [CROSS_ENTROPY_IGNORE_IDX if m else t for m, t in zip(r_masks, rejected_input_ids)]
