...
class ActorRolloutRefWorker(Worker):
    ...
    @register(dispatch_mode=Dispatch.ONE_TO_ALL)
    def save_checkpoint(self, local_path, hdfs_path=None, save_checkpoints = False):
        assert self._is_actor
        import torch
        if self._is_offload_param:
            load_fsdp_param_and_grad(module=self.actor_module_fsdp,
        ...
        if not save_checkpoints:
            ...
            if self.rank == 0:
                ...
                # with FSDP.state_dict_type(self.actor.actor_module, StateDictType.FULL_STATE_DICT, cfg):
                #     state_dict = self.actor.actor_module.state_dict()
                #     print(f'Saving actor checkpoint to {hf_local_path}')
                #     os.makedirs(hf_local_path, exist_ok=True)
                #     self.actor_module.save_pretrained(hf_local_path, state_dict=state_dict)
            ...
            with warnings.catch_warnings():
                ...
                with FSDP.state_dict_type(self.critic_module, StateDictType.SHARDED_STATE_DICT, state_dict_cfg, optim_cfg):
                    ...
                    if self.critic_lr_scheduler is not None:
                        lr_scheduler_state_dict = self.critic_lr_scheduler.state_dict()
                    else:
                        lr_scheduler_state_dict = None
